...
new_method = '''    `;
  ...
  renderLearningRecommendations(job, r) {
    ...
    const courseLib = [
      { id: 'c001', title: '密度泛函理论进阶', icon: 'atom', match: ['DFT','VASP','能带','态密度'] },
      { id: 'c002', title: '第一性原理计算实战', icon: 'flask-conical', match: ['DFT','VASP','结构优化','自洽'] },
      { id: 'c003', title: '拓扑绝缘体导论', icon: 'magnet', match: ['拓扑','能带','表面态'] },
      { id: 'c010', title: '分子动力学模拟入门', icon: 'dna', match: ['MD','GROMACS','LAMMPS'] },
      { id: 'c023', title: '材料科学基础', icon: 'flask-conical', match: ['材料','晶体','半导体'] }
    ];
    const articleLib = [
      { id: 'a001', title: 'MoS₂莫尔超晶格中的平带与强关联效应', tag: '拓扑材料', match: ['MoS₂','二维','能带'] },
      { id: 'a002', title: 'VASP计算中k点网格收敛性的系统研究', tag: '计算方法', match: ['VASP','k点','收敛'] },
      { id: 'a004', title: '石墨烯纳米带的能带工程与输运性质', tag: '低维材料', match: ['石墨烯','纳米带','输运'] },
      { id: 'a005', title: '水分子的氢键网络与相变机制', tag: '计算化学', match: ['水','分子','氢键'] }
    ];
    const nodeLib = {
...
if old in content:
    ...
    print("✅ renderLearningRecommendations方法已添加")
else:
    print("❌ 未找到匹配")
    idx = content.find('下载文件（真实功能')
    if idx >= 0:
        print("位置:", idx)
        print("上下文:", repr(content[max(0,idx-60):idx+40]))

with open(FILEPATH, 'w', encoding='utf-8') as f:
    f.write(content)
print("文件已保存, 大小:", len(content))
